...
if __name__ == "__main__":
    ...
    from_account = w3.toChecksumAddress("0xd16a3Be44242dE58AAA479E6da61697C150184f4")
    abi_build_index = json.loads(abi_build_index)
    store_var_contract = w3.eth.contract(address=w3.toChecksumAddress('0x95d52c2269c12b274c9c23dd9b25432debcd598d'), abi=abi_build_index)

    p_hex = hex(int(p))
    gas_estimate = store_var_contract.functions.setP(p_hex).estimateGas({"from": from_account})
    ...
    gas_estimate = store_var_contract.functions.set_CEDB_batch(u_list, C_0_list, C_1_list, C_2_list, y_list, len_test_CEDB).estimateGas({"from": from_account})
    tx_set_CEDB_batch_hash = store_var_contract.functions.set_CEDB_batch(u_list, C_0_list, C_1_list, C_2_list, y_list, len_test_CEDB).transact({"from": from_account, "gas": gas_estimate, "gasPrice": 0})
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_set_CEDB_batch_hash)
    print('---------set CEDB', tx_receipt.status, tx_receipt.gasUsed)
    gas_estimate = store_var_contract.functions.set_CEDB_tc_batch(u_list, tc_list, len_test_CEDB).estimateGas({"from": from_account})
    tx_CEDB_tc_batch_hash = store_var_contract.functions.set_CEDB_tc_batch(u_list, tc_list, len_test_CEDB).transact({"from": from_account, "gas": gas_estimate, "gasPrice": 0})
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_CEDB_tc_batch_hash)
    print('---------setCEDB_tc', tx_receipt.status, tx_receipt.gasUsed)
    gas_estimate = store_var_contract.functions.set_CEDB_delta(u_list, delta_list, len_test_CEDB).estimateGas({"from": from_account})
    tx_CEDB_delta_hash = store_var_contract.functions.set_CEDB_delta(u_list, delta_list, len_test_CEDB).transact(
        {"from": from_account, "gas": gas_estimate, "gasPrice": 0})
    tx_receipt = w3.eth.waitForTransactionReceipt(tx_CEDB_delta_hash)
    print('---------setCEDB_delta', tx_receipt.status, tx_receipt.gasUsed)
    gas_estimate = store_var_contract.functions.set_CEDB_y(u_list, y_list, len_test_CEDB).estimateGas({"from": from_account})
    ...
    gas_estimate = store_var_contract.functions.CBSIndex_add(test_CBSIndex, len_test_CBSIndex).estimateGas({"from": from_account})
    tx_CBSIndex_add_hash = store_var_contract.functions.CBSIndex_add(test_CBSIndex, len_test_CBSIndex).transact({"from": from_account, "gas": gas_estimate, "gasPrice": 0})
    ...
    gas_estimate = store_var_contract.functions.set_CPT_stc(CPT_l, CPT_stc, len_CPT).estimateGas({"from": from_account})
    ...
    gas_estimate = store_var_contract.functions.set_CPT_c(CPT_l, CPT_c, len_CPT).estimateGas({"from": from_account})
    ...
    gas_estimate = store_var_contract.functions.get_CPT_stc(l).estimateGas({"from": from_account})
    ...
    gas_estimate = store_var_contract.functions.get_CPT_c(l).estimateGas({"from": from_account})
    ...
    while_c = c
    while while_c > 0:
        gas_estimate = store_var_contract.functions.set_ST_w(while_c, test_xtoken[while_c], per_token_number).estimateGas({"from": from_account})
    ...
    gas_estimate = store_var_contract.functions.onchain_search(test_stag_w, st_c, c, per_token_number).estimateGas({"from": from_account})
